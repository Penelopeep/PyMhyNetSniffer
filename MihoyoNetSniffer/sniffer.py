	...
	def _packet_process_loop(self, override_func=None):
		...
		while True:
			...
			if self.protobuf_parser.get_packet_name(raw_packet.message_id) in self.banned_packets:
				del raw_packet
				continue
			if len(self.whitelisted_packets) > 0 and self.protobuf_parser.get_packet_name(raw_packet.message_id) not in self.whitelisted_packets:
				del raw_packet
				continue
			i += 1
			packet = ParsedPacket(
			...
			if not path.exists(".\\dumped"):
				os.mkdir(".\\dumped")
			with open(f'.\\dumped\\{i} {time} {self.protobuf_parser.get_packet_name(raw_packet.message_id)}.json', "w") as f:
